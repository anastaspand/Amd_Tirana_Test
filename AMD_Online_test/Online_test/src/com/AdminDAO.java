...
public class AdminDAO {
	...
	public static String addSection(String section){
		String query="insert into "+DBDetails.SECTION_TABLE+" ("+DBDetails.SECTION_COL+") values(?)";
	...
	public static JSONArray getSections(){
		String query="select * from "+DBDetails.SECTION_TABLE;
		...
		try{
			...
			while(rs.next()){
				...
				obj.put("section", rs.getString(DBDetails.SECTION_COL));
	...
	public static String deleteSection(String id){
		String query="delete from "+DBDetails.SECTION_TABLE+" where "+DBDetails.ID_COL+"=?";
	...
	public static JSONArray getResult(){
		String query="select * from "+DBDetails.USER_TABLE+" where retries<>0";
		JSONArray array= new JSONArray();
		JSONObject obj;
		...
		try{
			...
			ResultSet rs=ps.executeQuery();
			ResultSet rs1=null;
			query="select a.*, c.SECTION from "+DBDetails.RESULT_TABLE+" a inner join question b on a.Q_ID=b.ID inner join section c on b.Q_SET=c.id  where a."+DBDetails.USER_ID_COL+"=? and a."+DBDetails.RETRIES_COL+"=?";
			
			while(rs.next()){
				
				int retries = rs.getInt(DBDetails.RETRIES_COL);
				...
				for(int i=1;i<=retries;i++)
				{
					...
					while(rs1.next()){
						...
						obj.put("section", rs1.getString("SECTION"));
						
						obj.put("tester", rs1.getInt("ADMIN_ID") == -1 ? "Staff":"Admin");
					}
